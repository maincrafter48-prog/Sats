...
local function loadCfg()
    ...
    if ok and data and data.main then
        ...
        local okOld, dataOld = pcall(function() return inicfg.load(CFG_FILE_OLD) end)
...
-- обычный синхронный HTTP GET через ту же опциональную библиотеку
-- 'requests', что используется для курсов валют
...
function checkForUpdate(silent)
    ...
    lua_thread.create(function()
        ...
        if not body then
            local messages = {
                no_requests_lib = "\xed\xe5\xf2\x20\xe1\xe8\xe1\xeb\xe8\xee\xf2\xe5\xea\xe8\x20requests\x20\xe4\xeb\xff\x20HTTP",
