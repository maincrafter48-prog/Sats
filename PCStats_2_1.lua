...
local UPDATE_SCRIPT_URL  = UPDATE_RAW_BASE .. "PCStats_2.lua"
...
function main()
    ...
    -- Автопроверка обновлений при старте скрипта убрана: она была самым новым
    -- и наименее проверенным фоновым сетевым кодом в скрипте, и краши
    -- при заходе в игру/открытии меню начались именно после её появления.
    -- Теперь проверка версии происходит ТОЛЬКО вручную, по
    -- нажатию кнопки "Проверить обновление" в настройках (см.
    -- checkForUpdate выше) — то есть в конкретный, контролируемый момент, а не в фоне сама по себе.
