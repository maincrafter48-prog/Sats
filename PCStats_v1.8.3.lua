...
function Updater.fetch(url, dest, timeoutMs, minSize)
    ...
    while waited < timeoutMs do
        ...
        if knownTotal > 0 then
            ...
            -- размер неизвестен (сервер не прислал Content-Length) — как и
            -- раньше, ориентируемся на "файл перестал расти"
            sizeLooksComplete = lastSz >= minSize and stable >= 5
        end
...
function Updater.download(url, dest, onDone, minSize, timeoutMs)
    ...
    lua_thread.create(function()
        ...
        for i = 1, #urls do
            Updater.dlProg = 0
            local ok, err = Updater.fetch(urls[i], dest, timeoutMs, minSize)
            if ok then
...
function Updater.doDownload()
    ...
    -- полный файл может качаться дольше 12 секунд). Точную завершённость
    -- теперь всё равно решает Updater.fetch по реальному размеру с
    -- сервера — эти числа лишь запасная страховка, если сервер размер не
    -- прислал.
    Updater.download(Updater.scriptUrl(), tmp, function(ok, err)
        ...
        -- повреждена (обрыв на последних байтах, что Updater.fetch мог
        -- не отловить), поверх РАБОЧЕЙ версии оказывался битый .lua,
        -- reload() падал или MoonLoader молча отказывался грузить файл —
        -- и получался тот самый "процент дошёл до 100%, а толку ноль".
        -- Теперь файл сначала пытаются скомпилировать (не выполняя): при
        -- синтаксической ошибке обновление отменяется, а рабочая версия
        -- остаётся нетронутой.
        ...
        lua_thread.create(function()
            wait(900)
            local okReload = false
            if thisScript and thisScript() and thisScript().reload then
                okReload = pcall(function() thisScript():reload() end)
            end
            -- ФИКС: раньше результат reload() не проверялся вообще — если
            -- метод отсутствовал (старый MoonLoader) или падал, файл на
            -- диске уже был новым, но игра продолжала выполнять СТАРЫЙ
            -- код в памяти, и "О скрипте" вечно показывало версию до
            -- обновления, будто ничего не произошло.
            if not okReload then
                Updater.dlState = "error"
                Updater.err = "файл обновлён, но авто-перезапуск не сработал — перезайдите в игру"
            end
        end)
    end, 20000, 60000)
end
