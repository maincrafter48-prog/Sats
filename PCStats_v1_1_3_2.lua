...
local rateVCBuf  = imgui.new.int(0)
...
local _finShiftAnim       = 0.0   -- 0..1, текущая фаза анимации
local _finShiftAppliedPx  = 0.0   -- сколько пикселей сдвига уже применено в прошлый кадр
local _finShiftLastTime   = nil
...
local function fetchRatesViaCEF()
    ...
    if not isSampAvailable() then
        _cefLastResult = "\xf1\xe0\xec\xef \xed\xe5 \xe4\xee\xf1\xf2\xf3\xef\xe5\xed"
        return
    end
    _cefFetching     = true
    _phoneFetchState = "opening"
    _cefLastResult   = "\xee\xf2\xea\xf0\xfb\xe2\xe0\xe5\xec \xf2\xe5\xeb\xe5\xf4\xee\xed..."
    ...
    lua_thread.create(function()
        ...
        if not okCmd then
            ...
            _cefFetching     = false
            _cefLastResult   = "\xed\xe5 \xf3\xe4\xe0\xeb\xee\xf1\xfc \xee\xf2\xef\xf0\xe0\xe2\xe8\xf2\xfc \xea\xee\xec\xe0\xed\xe4\xf3 " .. cmd
            return
        end
        local waited = 0
        ...
        if _phoneFetchState then
            ...
            if failStage == "opening" then
                _cefLastResult = "\xed\xe5 \xf3\xe4\xe0\xeb\xee\xf1\xfc \xee\xef\xf0\xe5\xe4\xe5\xeb\xe8\xf2\xfc \xf1\xef\xe8\xf1\xee\xea \xe2\xea\xeb\xe0\xe4\xee\xea \xf2\xe5\xeb\xe5\xf4\xee\xed\xe0"
            elseif failStage == "last_tab" then
                _cefLastResult = "\xed\xe5 \xed\xe0\xe9\xe4\xe5\xed \xef\xf3\xed\xea\xf2 \\\"\xca\xf0\xe8\xef\xf2\xee\xe2\xe0\xeb\xfe\xf2\xe0\\\" \xed\xe0 \xef\xee\xf1\xeb\xe5\xe4\xed\xe5\xe9 \xe2\xea\xeb\xe0\xe4\xea\xe5"
            else
                _cefLastResult = "\xed\xe5 \xf3\xe4\xe0\xeb\xee\xf1\xfc \xf0\xe0\xe7\xee\xe1\xf0\xe0\xf2\xfc \xea\xf3\xf0\xf1\xfb \xe2 \xee\xf2\xea\xf0\xfb\xf2\xee\xec \xf0\xe0\xe7\xe4\xe5\xeb\xe5"
            end
            pcall(sampAddChatMessage, "{FF6666}[Stats] " .. tostring(_cefLastResult), -1)
...
local UPDATE_GH_FILE    = "PCStats_v1.1.1.lua"   -- <- путь к .lua файлу скрипта внутри репозитория
local UPDATE_GH_VERFILE = "version.txt"          -- <- необязательный лёгкий файл с одной строкой-версией

local SCRIPT_VERSION = "1.1.3" -- должно совпадать с script_version() в начале файла
...
local function applyWikiRatesForServer(serverName, silent)
    ...
    if r.vc  then cfg.rateVC  = r.vc;  rateVCBuf[0]  = math.floor(r.vc  + 0.5) end
    if r.btc then cfg.rateBTC = r.btc; rateBTCBuf[0] = math.floor(r.btc + 0.5) end
    if r.az  then cfg.rateAZ  = r.az;  rateAZBuf[0]  = math.floor(r.az  + 0.5) end
    if r.eur then cfg.rateEUR = r.eur; rateEURBuf[0] = math.floor(r.eur + 0.5) end
    if r.asc then cfg.rateASC = r.asc; rateASCBuf[0] = math.floor(r.asc + 0.5) end
    saveCfg()
    if not silent then
...
local function drawFinanceSettingsBlock(r, g, b)
    ...
    local sbc = _financeSettingsOpen and _fsAc or _fsOn
...
local function drawFinanceSettingsPanel()
    ...
    if not _financeSettingsDetached then
        ...
        imgui.SetNextWindowSize(imgui.ImVec2(panelW, _mainWinSize.y), imgui.Cond.Once)
        if _financeSettingsPos then
            ...
            imgui.SetNextWindowPos(imgui.ImVec2(_mainWinPos.x + _mainWinSize.x + S(10), _mainWinPos.y), imgui.Cond.Once)
        end
    end
...
imgui.OnFrame(
    ...
    function(self)
        ...
        do
            local tnow = os.clock()
            if _finShiftLastTime == nil then _finShiftLastTime = tnow end
            local dt = tnow - _finShiftLastTime
            _finShiftLastTime = tnow
            if dt < 0 or dt > 0.5 then dt = 0 end -- защита от скачков (первый кадр / лаги)

            local target = (_financeSettingsOpen and not _financeSettingsDetached) and 1.0 or 0.0
            local speed  = 6.0 -- скорость анимации, полный сдвиг за ~1/speed сек
            if _finShiftAnim < target then
                _finShiftAnim = math.min(target, _finShiftAnim + dt*speed)
            elseif _finShiftAnim > target then
                _finShiftAnim = math.max(target, _finShiftAnim - dt*speed)
            end

            local finShiftPx = S(330) * _finShiftAnim -- ширина панели (320) + отступ (10)
            local deltaPx    = finShiftPx - _finShiftAppliedPx
            _finShiftAppliedPx = finShiftPx

            if math.abs(deltaPx) > 0.01 and _mainWinPos then
                imgui.SetNextWindowPos(imgui.ImVec2(_mainWinPos.x - deltaPx, _mainWinPos.y), imgui.Cond.Always)
            end
        end
        ...
        pcall(function()
            ...
            elseif _phoneFetchState == "last_tab" then
                ...
                if idx then
                    ...
                    _cefFetching = false
                    _cefLastResult = "\xed\xe5 \xed\xe0\xe9\xe4\xe5\xed \xef\xf3\xed\xea\xf2 \\\"\xca\xf0\xe8\xef\xf2\xee\xe2\xe0\xeb\xfe\xf2\xe0\\\" \xed\xe0 \xef\xee\xf1\xeb\xe5\xe4\xed\xe5\xe9 \xe2\xea\xeb\xe0\xe4\xea\xe5"
                    pcall(sampAddChatMessage, "{FF9900}[Stats] " .. tostring(_cefLastResult), -1)
            ...
            elseif _phoneFetchState == "crypto_menu" then
                ...
                _phoneFetchState = false
                if got then
                    _cefLastResult = "\xea\xf3\xf0\xf1\xfb \xee\xe1\xed\xee\xe2\xeb\xe5\xed\xfb \xe8\xe7 \xf2\xe5\xeb\xe5\xf4\xee\xed\xe0"
