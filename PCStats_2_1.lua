...
local UPDATE_SCRIPT_URL  = UPDATE_RAW_BASE .. "PCStats_2_1.lua"
local UPDATE_VERSION_URL = UPDATE_RAW_BASE .. "version.txt"
-- changelog.txt в корне репозитория — обычный текстовый файл (UTF-8),
-- содержимое которого автоматически подтягивается в меню обновления;
-- редактируется прямо на GitHub, скрипт ничего не парсит кроме сырого текста
local UPDATE_CHANGELOG_URL = UPDATE_RAW_BASE .. "changelog.txt"
...
local cfg = {
    ...
    lastTab = 1,
    -- ── настройки проверки обновлений: оба тумблера включены по
    -- умолчанию при первом запуске (когда settings.ini ещё нет) ──
    -- уведомление в чат о доступной новой версии (при входе в игру и
    -- после ручной/автоматической проверки версии)
    updateNotify = true,
    -- автоматически скачивать и устанавливать новую версию, если она найдена
...
local function deserializeCustomColors(s)
    ...
    for id, rgb in tostring(s):gmatch("([^=;]+)=([^;]+)") do
        ...
        if rr then
            local r,g,b = tonumber(rr), tonumber(gg), tonumber(bb)
            -- защита: значения из файла настроек должны быть в 0..1,
            -- иначе битые/экстремальные числа могли бы напрямую попасть
            -- в нативный рендер цвета (imgui ColorEdit/ImVec4)
            if r and g and b then
                r = math.max(0, math.min(1, r))
                g = math.max(0, math.min(1, g))
                b = math.max(0, math.min(1, b))
                customColors[id] = {r, g, b}
            end
        end
    end
end
...
local function applyCfgData(m)
    ...
    if m.updateNotify == nil then cfg.updateNotify = true else cfg.updateNotify = m.updateNotify == "true" end
    if m.updateAutoInstall == nil then cfg.updateAutoInstall = true else cfg.updateAutoInstall = m.updateAutoInstall == "true" end
end
...
local function saveCfg()
    ...
    pcall(function()
        inicfg.save({main={
            ...
            updateNotify      = tostring(cfg.updateNotify),
            updateAutoInstall = tostring(cfg.updateAutoInstall),
...
-- ── история изменений (changelog.txt с GitHub): текст подтягивается
-- вместе с проверкой версии и отображается прямо в карточке
-- "Обновления" меню, без какого-либо парсинга — просто сырой текст
-- файла (ожидается UTF-8, т.к. это стандартная кодировка файлов на
-- GitHub; для imgui это как раз то, что нужно — рендерится без u8()) ──
local _changelogFetching = false
local _changelogText     = nil
local _changelogStatus   = nil -- сырые CP1251-байты, для чата/статуса

-- обычный синхронный HTTP GET через библиотеку 'requests'. Раньше здесь
-- также был вариант через нативную функцию downloadUrlToFile (асинхронный
-- колбэк + цикл ожидания в фоновом потоке) — убран, т.к. именно после его
-- добавления начались краши игры при открытии меню; это самая рискованная
-- и наименее проверенная часть из всего, что менялось, поэтому её просто
-- отключаем, а не пытаемся чинить вслепую. Если requests не установлена —
-- проверка обновлений просто ничего не делает (без краша).
...
-- ── подтягивает changelog.txt с GitHub (текст правится прямо в
-- репозитории, скрипт просто показывает его как есть). silent=true —
-- без сообщений об ошибке в чат (используется при автопроверке) ──
local function fetchChangelog(silent)
    if _changelogFetching then return end
    _changelogFetching = true
    lua_thread.create(function()
        local okCall, body, err = pcall(httpGetUrl, UPDATE_CHANGELOG_URL)
        if not okCall then body, err = nil, "internal_error" end
        _changelogFetching = false
        if not body then
            _changelogStatus = "\xed\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xe7\xe0\xe3\xf0\xf3\xe7\xe8\xf2\xfc\x20\xf1\xef\xe8\xf1\xee\xea\x20\xe8\xe7\xec\xe5\xed\xe5\xed\xe8\xe9: " .. tostring(err)
            if not silent then
                pcall(sampAddChatMessage, "{FF6666}[PC Stats] " .. _changelogStatus, -1)
            end
            return
        end
        -- отсекаем BOM/лишние переносы строк по краям
        body = body:gsub("^\239\187\191", ""):gsub("^%s+", ""):gsub("%s+$", "")
        _changelogText   = body
        _changelogStatus = nil
    end)
end
...
function checkForUpdate(silent)
    ...
    lua_thread.create(function()
        ...
        -- защита от ложного срабатывания: если вместо version.txt пришла
        -- страница ошибки (например 404 от GitHub из-за неверного пути/
        -- приватного репозитория), в тексте может случайно найтись число
        -- вида "404" — не должно приниматься за версию. Настоящий файл
        -- version.txt короткий (< 40 байт) и почти целиком состоит из
        -- самого номера версии
        if remoteVer and (#body > 40 or #remoteVer < #body - 5) then
            remoteVer = nil
        end
        ...
        -- changelog подтягиваем при каждой проверке версии, чтобы он всегда
        -- отображал актуальный текст с GitHub, даже если новой версии нет
        pcall(fetchChangelog, true)
        if isNewerVersion(remoteVer, SCRIPT_VER) then
            ...
            -- уведомление в чат о новой версии показывается только если
            -- включён тумблер "Уведомления о новой версии" в настройках
            if cfg.updateNotify then
                pcall(sampAddChatMessage, "{00FF88}[PC Stats] " .. _updStatus, -1)
            end
            -- автообновление: если включён тумблер "Автоматическое
            -- обновление" — скрипт сам скачивает и ставит новую версию,
            -- без участия игрока (запускается сразу после проверки)
            if cfg.updateAutoInstall then
...
function downloadAndApplyUpdate()
    ...
    lua_thread.create(function()
        ...
        pcall(saveCfg)
        -- принудительно закрываем окно меню перед перезапуском: если
        -- обновление применяется, пока imgui-окно открыто/рисуется,
        -- reload скрипта посреди кадра мог крашить всю игру ──
        pcall(function() St.winOpen = false end)
        _updDownloading = false
        wait(500)
        selfReloadScript()
    end)
...
local function drawAbout(h)
    ...
    if St._resetSettScroll then imgui.SetScrollY(0) end
        ...
        -- ── карточка настроек обновлений: два тумблера — уведомления о
        -- новой версии в чат и автоматическая установка обновления,
        -- оба включены по умолчанию при первом запуске ──
        secTitle(u8"\xcd\xe0\xf1\xf2\xf0\xee\xe9\xea\xe8\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe9")
        aboutCard("##updsettingscard", 76, function(aw, ch)
            imgui.SetWindowFontScale(aboutBaseScale)
            imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(12)))
            if drawToggleSwitch("##updNotifySw", cfg.updateNotify) then
                cfg.updateNotify = not cfg.updateNotify
                saveCfg()
            end
            imgui.SameLine(0, SFtext(8))
            imgui.TextColored(iv4(0.85,0.87,0.95,1.0), u8"\xd3\xe2\xe5\xe4\xee\xec\xeb\xe5\xed\xe8\xff\x20\xee\x20\xed\xee\xe2\xee\xe9\x20\xe2\xe5\xf0\xf1\xe8\xe8")

            imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(42)))
            if drawToggleSwitch("##updAutoSw", cfg.updateAutoInstall) then
                cfg.updateAutoInstall = not cfg.updateAutoInstall
                saveCfg()
            end
            imgui.SameLine(0, SFtext(8))
            imgui.TextColored(iv4(0.85,0.87,0.95,1.0), u8"\xc0\xe2\xf2\xee\xec\xe0\xf2\xe8\xf7\xe5\xf1\xea\xee\xe5\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5")
            imgui.SetWindowFontScale(aboutBaseScale)
        end)
        ...
        -- ── карточка "Список изменений": текст changelog.txt с GitHub,
        -- подтягивается автоматически вместе с проверкой версии (см.
        -- checkForUpdate/fetchChangelog) — редактируется прямо в
        -- репозитории, скрипт просто показывает его как есть ──
        secTitle(u8"\xd1\xef\xe8\xf1\xee\xea\x20\xe8\xe7\xec\xe5\xed\xe5\xed\xe8\xe9")
        aboutCard("##changelogcard", 160, function(aw, ch)
            imgui.SetWindowFontScale(aboutBaseScale)
            imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(10)))
            imgui.PushTextWrapPos(aw - SFtext(16))
            if _changelogFetching and not _changelogText then
                imgui.TextColored(iv4(0.55,0.62,0.80,1.0), u8"\xc7\xe0\xe3\xf0\xf3\xe7\xea\xe0\x2e\x2e\x2e")
            elseif _changelogText and _changelogText ~= "" then
                -- текст changelog.txt ожидается в UTF-8 (стандартная
                -- кодировка файлов на GitHub) — рендерится в imgui как
                -- есть, без u8(), т.к. u8() конвертирует из CP1251
                imgui.TextColored(iv4(0.85,0.87,0.95,1.0), _changelogText)
            elseif _changelogStatus then
                imgui.TextColored(iv4(0.95,0.55,0.30,1.0), u8(_changelogStatus))
            else
                imgui.TextColored(iv4(0.45,0.48,0.56,1.0),
                    u8"")
            end
            imgui.PopTextWrapPos()
            imgui.SetWindowFontScale(aboutBaseScale)
        end)
    ...
    function(self)
        ...
        do
            ...
            imgui.BeginChild("##titlebar", imgui.ImVec2(aw0, th0), false)
                local titleStr = u8"  PC Stats  v1.1.6"
...
function main()
    ...
    St.activeTab = 1 -- меню всегда открывается на первой вкладке (было cfg.lastTab)
    ...
    function toggleMenuWindow()
        ...
        if St.winOpen then
            _sw_win_init = nil
            St.activeTab = 1 -- меню всегда открывается на первой вкладке
            requestStats()
        else
            St.activeTab = 1
        end
    ...
    -- 5. Сообщение в чат — ждём РЕАЛЬНЫЙ спавн игрока
    -- Автопроверка обновлений при входе в игру: выполняется тем же
    -- безопасным путём, что и автоподстановка курсов валют ниже — уже
    -- ПОСЛЕ дождавшегося спавна игрока и с задержкой, синхронным HTTP
    -- через 'requests' (никакой асинхронной downloadUrlToFile, которая
    -- раньше и вызывала крашы). Сообщение о новой версии показывается
    -- в чат только при cfg.updateNotify (см. checkForUpdate выше).
    ...
    lua_thread.create(function()
        ...
        pcall(sampAddChatMessage,
            "{00FF88}[MSW v1.1.6] {FFFFFF}PC Stats | Cmd: {00FF88}/sw", -1)
        -- автопроверка обновлений при входе в игру (silent=true — ошибки
        -- сети молча игнорируются, но уведомление о НАЙДЕННОЙ новой
        -- версии всё равно уйдёт в чат, если включён cfg.updateNotify;
        -- см. checkForUpdate)
