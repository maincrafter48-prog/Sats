...
function Updater.log(msg)
    pcall(sampAddChatMessage, "{00AAFF}[PC Stats | Update] {FFFFFF}" .. tostring(msg), -1)
end
...
function Updater.download(url, dest, onDone)
    url = tostring(url or ""); dest = tostring(dest or "")
    if url == "" or dest == "" then
        if onDone then onDone(false, "empty url") end
        return
    end
    -- 1) MoonLoader
    if type(downloadUrlToFile) == "function" then
        pcall(downloadUrlToFile, url, dest, function() end)
        lua_thread.create(function()
            local waited = 0
            while waited < 20000 do
                wait(400); waited = waited + 400
                Updater.dlProg = math.min(90, math.floor(waited / 200))
                local f = io.open(dest, "rb")
                if f then
                    local sz = f:seek("end"); f:close()
                    if sz and sz > 20 then
                        if onDone then onDone(true, nil) end
                        return
                    end
                end
            end
            if onDone then onDone(false, "timeout downloadUrlToFile") end
        end)
        return
    end
    -- 2) requests
    local okR, requests = pcall(require, "requests")
    if okR and requests and (requests.get or requests.request) then
        lua_thread.create(function()
            Updater.dlProg = 10
            local ok, resp
            if requests.get then
                ok, resp = pcall(requests.get, url, {timeout = 20})
            else
                ok, resp = pcall(requests.request, "GET", url, nil)
            end
            Updater.dlProg = 70
            if ok and resp and (resp.status_code == 200 or resp.status == 200) then
                local body = resp.text or resp.content or ""
                if type(body) == "string" and #body > 20 then
                    local out = io.open(dest, "wb")
                    if out then out:write(body); out:close()
                        Updater.dlProg = 100
                        if onDone then onDone(true, nil) end
                        return
                    end
            ...
            if onDone then onDone(false, "HTTP fail") end
        end)
        return
    end
    -- 3) urlmon
    if ffi then
        ...
        if urlmon and urlmon.URLDownloadToFileA then
            ...
            if onDone then onDone(false, "urlmon hr=" .. tostring(hr)) end
            return
        end
    end
    if onDone then onDone(false, "нет загрузчика (downloadUrlToFile / requests / urlmon)") end
end

function Updater.check(manual)
    if Updater.checking then
        if manual then Updater.log("\xf3\xe6\xe5\x20\xef\xf0\xee\xe2\xe5\xf0\xff\xfe\x2e\x2e\x2e") end
        return
    end
    if not Updater.isConfigured() then
        ...
        if manual then Updater.log(Updater.err) end
        return
    end
    Updater.checking = true
    Updater.status = "checking"
    Updater.err = ""
    if manual then Updater.log("\xef\xf0\xee\xe2\xe5\xf0\xff\xfe\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xff\x2e\x2e\x2e") end
    local tmp = (CFG_DIR or "moonloader/config/PCStats") .. "/_upd_ver.txt"
    pcall(function() if ensureCfgDir then ensureCfgDir() end end)
    Updater.download(Updater.versionUrl(), tmp, function(ok, err)
        if not ok then
            Updater.checking = false
            Updater.status = "error"
            Updater.err = tostring(err or "download")
            Updater.log("\xed\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xf1\xea\xe0\xf7\xe0\xf2\xfc\x20\x76\x65\x72\x73\x69\x6f\x6e\x2e\x74\x78\x74\x3a\x20" .. Updater.err)
            return
        end
        local raw = Updater.readFile(tmp) or ""
        pcall(os.remove, tmp)
        local remote = (raw:match("([%d%.]+)")) or ""
        remote = remote:gsub("%s+", "")
        if remote == "" then
            Updater.checking = false
            Updater.status = "error"
            Updater.err = "пустой version.txt"
            return
        end
        ...
        if remote == tostring(SCRIPT_VER) or not Updater.verLt(SCRIPT_VER, remote) then
            Updater.checking = false
            Updater.status = "ok"
            if manual then Updater.log("\xe0\xea\xf2\xf3\xe0\xeb\xfc\xed\xe0\xff\x20\xe2\xe5\xf0\xf1\xe8\xff\x3a\x20" .. SCRIPT_VER) end
            return
        end
        Updater.checking = false
        Updater.status = "outdated"
        Updater.log(string.format("\xe4\xee\xf1\xf2\xf3\xef\xed\xee\x20\x76\x25\x73\x20\x28\xf1\xe5\xe9\xf7\xe0\xf1\x20\x76\x25\x73\x29", remote, SCRIPT_VER))
    end)
end

function Updater.doDownload()
    if Updater.dlState == "downloading" then return end
    ...
    Updater.dlProg = 0
    Updater.err = ""
    local tmp = (CFG_DIR or "moonloader/config/PCStats") .. "/_upd_script.lua"
    pcall(function() if ensureCfgDir then ensureCfgDir() end end)
    Updater.log("\xf1\xea\xe0\xf7\xe8\xe2\xe0\xfe\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5\x2e\x2e\x2e")
    Updater.download(Updater.scriptUrl(), tmp, function(ok, err)
        if not ok then
            Updater.dlState = "error"
            Updater.err = tostring(err or "fail")
            Updater.log("\xee\xf8\xe8\xe1\xea\xe0\x20\xf1\xea\xe0\xf7\xe8\xe2\xe0\xed\xe8\xff\x3a\x20" .. Updater.err)
            return
        end
        ...
        Updater.log("\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xee\x20\xe4\xee\x20\x76" .. tostring(Updater.latest or "?") .. "\x2c\x20\xef\xe5\xf0\xe5\xe7\xe0\xe3\xf0\xf3\xe7\xea\xe0\x2e\x2e\x2e")
        lua_thread.create(function()
            wait(900)
            pcall(function()
...
local function applyCfgData(m)
    ...
    cfg.updateCheckOnStart = toBool(m.updateCheckOnStart, false)
...
local function drawAboutInner(h)
    if St._resetSettScroll then imgui.SetScrollY(0) end
        ...
        aboutCard("##updcard", 88, function(aw, ch)
            imgui.SetWindowFontScale(aboutBaseScale)
            ...
            if us == "ok" then
                imgui.TextColored(iv4(0.30,0.95,0.40,1.0), u8"\x90 \xc0\xea\xf2\xf3\xe0\xeb\xfc\xed\xe0\xff \xe2\xe5\xf0\xf1\xe8\xff")
            elseif us == "checking" then
                imgui.TextColored(iv4(0.40,0.85,1.0,1.0), u8"\xcf\xf0\xee\xe2\xe5\xf0\xea\xe0...")
            elseif us == "outdated" then
                imgui.TextColored(iv4(1.0,0.75,0.15,1.0), u8"\x94 \xc4\xee\xf1\xf2\xf3\xef\xed\xee v" .. tostring(ul or "?"))
            ...
            if dls == "downloading" then
                imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(50)))
            ...
            imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(68)))
            if not Updater.isConfigured() then
                imgui.TextColored(iv4(1.0,0.55,0.25,1.0), u8"\xc7\xe0\xe4\xe0\xe9 PCS_GH_OWNER / REPO \xe2 \xed\xe0\xf7\xe0\xeb\xe5 \xf1\xea\xf0\xe8\xef\xf2\xe0")
            else
                imgui.TextColored(iv4(0.45,0.50,0.58,1.0), u8"GitHub: " .. tostring(PCS_GH_OWNER) .. "/" .. tostring(PCS_GH_REPO))
            end
            imgui.SetWindowFontScale(aboutBaseScale)
        end)
