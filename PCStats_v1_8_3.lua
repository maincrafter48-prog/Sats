...
-- зеркала: raw.githubusercontent -> jsDelivr -> raw.githack
...
function Updater.fetch(url, dest, timeoutMs, minSize)
    ...
    while waited < timeoutMs do
        ...
        if (dataEnd or allEnd) and lastSz >= minSize then break end
        if lastSz >= minSize and stable >= 5 then break end
        if allEnd and waited > 2000 then break end
        if Updater.dlProg < 90 then
            Updater.dlProg = math.max(Updater.dlProg, math.min(90, math.floor(waited * 90 / timeoutMs)))
        end
    end
...
function Updater.download(url, dest, onDone, minSize)
    ...
    lua_thread.create(function()
        ...
        for i = 1, #urls do
            Updater.dlProg = 0
            local ok, err = Updater.fetch(urls[i], dest, 12000, minSize)
            if ok then
...
function Updater.check(manual)
    ...
    if manual then Updater.log(Updater.versionUrl()) end
