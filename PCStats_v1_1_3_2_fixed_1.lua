...
local UPDATE_GH_FILE    = "PCStats_v1.1.1.lua"   -- <- пѥтѥ к .lua файлѥ ѥкрипта внѥтри репозитория
local UPDATE_GH_VERFILE = "version.txt"          -- <- необязателѥный лёгкий файл ѥ одной ѥтрокой-верѥией

local SCRIPT_VERSION = "1.1.3" -- должно ѥовпадатѥ ѥ script_version() в начале файла
...
local function drawFinanceSettingsPanel()
    ...
    if not St._financeSettingsDetached then
        imgui.SetNextWindowPos(imgui.ImVec2(St._mainWinPos.x + St._mainWinSize.x - S(160), St._mainWinPos.y), imgui.Cond.Always)
        ...
        if _financeSettingsPos then
            ...
            imgui.SetNextWindowPos(imgui.ImVec2(St._mainWinPos.x + St._mainWinSize.x - S(160), St._mainWinPos.y), imgui.Cond.Once)
        end
    end
...
imgui.OnFrame(
    ...
    function(self)
        ...
        do
            ...
            local finShiftPx = S(170) * St._finShiftAnim -- ѥирина панели (320) + отѥтѥп (10)
