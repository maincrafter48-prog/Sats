...
local cfg = {
    ...
    -- ── Продление отеля (вкладка "Налоги" → "Отель"), по просьбе: та же
    -- логика автооплаты по кулдауну, что и у налогов ──
    hotelAutoEnabled       = false, -- автопродление по таймеру вкл/выкл
    hotelAutoIntervalHours = 24,    -- через сколько часов повторять автопродление (1..120, см. UI)
    hotelLastPayTime       = 0,     -- os.time() последнего успешного продления (своего)
    hotelLastPayHours      = 0,     -- сколько часов продлили в последний раз
}
...
local function applyCfgData(m)
    ...
    -- ФИКС (по просьбе): возможность вручную менять фон скрипта убрана —
    -- фон всегда идёт от текущей темы, старое сохранённое значение (если
    -- есть в файле настроек с прошлых версий) больше не читается
    cfg.winBgR        = -1
    cfg.winBgG        = -1
    cfg.winBgB        = -1
    ...
    -- ── Продление отеля ──
    cfg.hotelAutoEnabled       = toBool(m.hotelAutoEnabled, false)
    cfg.hotelAutoIntervalHours = clampNum(m.hotelAutoIntervalHours, 1, 120, 24)
    cfg.hotelLastPayTime       = clampNum(m.hotelLastPayTime, 0, 99999999999, 0)
    cfg.hotelLastPayHours      = clampNum(m.hotelLastPayHours, 0, 999999, 0)
...
local function saveCfg()
    ...
    pcall(function()
        inicfg.save({main={
            ...
            hotelAutoEnabled       = tostring(cfg.hotelAutoEnabled),
            hotelAutoIntervalHours = tostring(cfg.hotelAutoIntervalHours),
            hotelLastPayTime       = tostring(cfg.hotelLastPayTime),
            hotelLastPayHours      = tostring(cfg.hotelLastPayHours),
...
do
    ...
    function PcsNotifyManager:_draw_toast(n)
        ...
        -- ── ЗАПАСНОЙ ВАРИАНТ: если в этой сборке mimgui вдруг нет
        -- GetForegroundDrawList — старое поведение через окно (со всеми
        -- известными "некликабельными" флагами разом, см. pcs_toast_flags
        -- выше). Хуже гарантии по мышке, но хотя бы не теряем сами
        -- уведомления.
        imgui.SetNextWindowPos(imgui.ImVec2(self._anchor_x, n.y), imgui.Cond.Always)
        imgui.SetNextWindowSize(imgui.ImVec2(mcfg.width, n.height), imgui.Cond.Always)
        pcs_safe_call(imgui.SetNextWindowBgAlpha, 0)

        local pushedBorder = pcall(imgui.PushStyleColor, imgui.Col.Border, iv4(0,0,0,0))

        local began = false
        local ok = pcall(function()
            if imgui.Begin("##pcsToast"..tostring(n.id), nil, PCS_TOAST_FLAGS) then
                began = true
                pcs_safe_call(imgui.SetWindowFontScale, mcfg.font_scale or 1.0)

                local wdl = imgui.GetWindowDrawList()
                local wpmin = imgui.GetWindowPos()
                local wpmax = imgui.ImVec2(wpmin.x + mcfg.width, wpmin.y + n.height)

                pcs_safe_call(wdl.AddRectFilled, wdl,
                    imgui.ImVec2(wpmin.x+3, wpmin.y+5), imgui.ImVec2(wpmax.x+3, wpmax.y+5),
                    pcs_pack_color(0, 0, 0, 0.35*a), mcfg.corner_radius)

                local bg = mcfg.bg_color
                wdl:AddRectFilled(wpmin, wpmax, pcs_pack_color(bg[1],bg[2],bg[3],bg[4]*a), mcfg.corner_radius)

                if cfg.rainbowBorder and (mcfg.border_size or 0) > 0 then
                    local hue = (os.clock() % 4.0) / 4.0
                    local hr, hg, hb = hsv2rgb(hue, 0.75, 1.0)
                    pcs_safe_call(wdl.AddRect, wdl, wpmin, wpmax,
                        pcs_pack_color(hr, hg, hb, a),
                        mcfg.corner_radius, 0, mcfg.border_size)
                end

                local c = def.color
                local smin = imgui.ImVec2(wpmin.x+3, wpmin.y+4)
                local smax = imgui.ImVec2(wpmin.x+3+mcfg.accent_width, wpmax.y-4)
                wdl:AddRectFilled(smin, smax, pcs_pack_color(c[1],c[2],c[3],a), mcfg.accent_width/2)

                imgui.SetCursorPosX(mcfg.padding_x + mcfg.accent_width + 6)
                imgui.SetCursorPosY(mcfg.padding_y)

                imgui.PushStyleColor(imgui.Col.Text, iv4(c[1],c[2],c[3],a))
                pcs_safe_call(imgui.PushTextWrapPos, mcfg.width - mcfg.padding_x)
                imgui.TextWrapped(def.label)
                pcs_safe_call(imgui.PopTextWrapPos)
                imgui.PopStyleColor()

                imgui.SetCursorPosX(mcfg.padding_x + mcfg.accent_width + 6)
                local tc = mcfg.text_color
                imgui.PushStyleColor(imgui.Col.Text, iv4(tc[1],tc[2],tc[3],(tc[4] or 1)*a))
                pcs_safe_call(imgui.PushTextWrapPos, mcfg.width - mcfg.padding_x)
                imgui.TextWrapped(n.text)
                pcs_safe_call(imgui.PopTextWrapPos)
                imgui.PopStyleColor()

                local measured_y = pcs_safe_call(imgui.GetCursorPosY)
                if measured_y then
                    local real_h = measured_y + mcfg.padding_y
                    if real_h > n.height then n.height = real_h
                    elseif math.abs(real_h - n.height) > 1 then n.height = real_h end
                end
            end
        end)

        if began then pcall(imgui.End) end
        if pushedBorder then pcall(imgui.PopStyleColor) end

        if not ok then
            n.state = PCS_STATE.DEAD
        end
    end
...
local SECTION_DEFS = {
    ...
    { label = ICON_SACK.." "..u8"\xcd\xe0\xeb\xee\xe3\xe8",             icon=ICON_SACK, name=u8"\xcd\xe0\xeb\xee\xe3\xe8",             r=1.0,g=0.65,b=0.15,
      -- ФИКС (по просьбе "перенести все функции вкладки 'Отель' во
      -- вкладку 'Оплата налогов'"): отдельной под-вкладки "Отель"
      -- (tab=7) больше нет в списке кнопок — её содержимое теперь
      -- рисуется СРАЗУ ПОСЛЕ содержимого налогов, внутри той же
      -- вкладки tab=6 (см. drawTaxes() ниже, вызывает drawHotelInner()
      -- вторым блоком в том же скролящемся окне). Сама функция
      -- drawHotel()/номер tab=7 в диспетчере вкладок оставлены (не
      -- удалены) на случай, если где-то ещё в файле остался переход на
      -- них по старой памяти — так безопаснее, чем чистить все ссылки.
      tabs = {
...
-- ── буферы вкладки "Отель" (по просьбе: та же схема, что у "Налоги") ──
St.hotelAutoBuf       = imgui.new.bool(false)
St.hotelIntervalBuf   = imgui.new.int(24)
...
-- ============================================================
--  АВТООПЛАТА ОТЕЛЯ (по просьбе: система продления номера, по аналогии с
--  автооплатой налогов выше). Сервер открывает диалог со списком
--  фиксированных вариантов продления ("1. 12 часов" / "2. 24 часов" /
--  "3. 48 часов" / "4. 96 часов") и строкой "Можно продлить на: N часов" —
--  это сколько ещё МОЖНО добавить сейчас (лимит). Логика: берём САМЫЙ
--  БОЛЬШОЙ вариант из списка, который не превышает этот лимит, жмём его,
--  и как только сервер обновит экран (лимит уменьшится на кол-во часов),
--  открываем меню снова и повторяем — пока остаток не станет меньше
--  минимального варианта в списке (тогда дальше платить нечем, и скрипт
--  просто закрывает телефон и пишет в чат итоговый остаток часов).
-- ФИКС/ДОГАДКА: id диалога продления зафиксирован (26148 — прислано
-- пользователем), а вот имя иконки приложения "Отель" на телефоне не
-- было указано явно — используется то же слово ("Отель"), которым сама
-- игра называет это поле в /stats (см. p.hotel выше по файлу). Если
-- реальная иконка на телефоне называется иначе (например "Гостиница"),
-- поправить HOTEL_ICON_NEEDLE ниже.
-- ============================================================
HOTEL_DIALOG_ID       = 26148
-- ОТКАТ ФИКСА (мой прошлый вариант был ошибочным): "ID: 6565" из вашего
-- описания — это ID самого ДИАЛОГА SA-MP (недаром рядом указан "Style: 2" —
-- это поле style из onShowDialog, а не код приложения), а не номер для
-- пакета "launchedApp|N", каким пользуются налоги (24/5656 — те коды
-- были подтверждены отдельно, именно для формата этого пакета). Отправка
-- "launchedApp|6565" была отправкой мусорного/неверного пакета — сервер
-- реагировал на него непредсказуемо, отсюда и "только 2 действия и
-- дальше тишина", и сам крash при переключении вкладки (ломался стейт
-- телефона). Возвращаемся к безопасному способу: открываем телефон и
-- ждём, пока onShowDialog найдёт иконку "Отель" по тексту — id=6565
-- используется только как ДОПОЛНИТЕЛЬНАЯ подсказка/лог (см. ниже), а не
-- как пакет для отправки.
HOTEL_HOME_DIALOG_ID  = 6565 -- id экрана, который приходит сразу после открытия телефона (для справки/лога)
HOTEL_ICON_NEEDLE     = "\xce\xf2\xe5\xeb\xfc" -- "Отель" (поиск по тексту)
HOTEL_STEP_DELAY      = 200
HOTEL_POST_PAY_WAIT_MS = 900
HOTEL_TIMEOUT_SEC     = 18
HOTEL_NAV_MAX_ATTEMPTS = 5
HOTEL_ERR_WORD_1      = "\xce\xf8\xe8\xe1\xea\xe0"  -- "Ошибка"
HOTEL_ERR_WORD_2      = "\xec\xe0\xea\xf1\xe8\xec\xf3\xec" -- "максимум"
_hotelState = 0 -- 0=простой,1=ищем иконку "Отель" на телефоне,2=обрабатываем экран продления (id=26148)
_hotelIsAuto = false
_hotelNavAttempts = 0
_hotelLastOpts = nil -- последний распарсенный список {часы=индекс_в_списке}, нужен обработчику ошибки в чате
-- сколько часов реально продлено за текущий "сеанс" payHotelNow() — оплата
-- может состоять из НЕСКОЛЬКИХ шагов подряд (см. цикл открытия меню заново
-- ниже), поэтому копим сумму и пишем в лог ОДНОЙ записью, когда сеанс
-- завершается (лимит исчерпан либо поймана ошибка сервера)
_hotelSessionHours = 0

-- ============================================================
--  ЛОГ ПРОДЛЕНИЙ ОТЕЛЯ (по просьбе: чтобы оплаты отеля тоже было видно
--  в логах — та же схема персистентного лога, что и TX-таблица у
--  налогов выше, только вместо суммы денег храним кол-во оплаченных
--  часов, т.к. сервер не присылает цену продления в тексте диалога)
-- ============================================================
-- ФИКС "main function has more than 200 local variables": файл и так
-- упёрся в лимит Lua 5.1 на локальные переменные главного чанка (см.
-- комментарий у PD выше) — HT, как и _hotelState/HOTEL_DIALOG_ID рядом,
-- намеренно объявлена БЕЗ "local", чтобы не расходовать лишний слот
HT = {}
HT.LOG_FILE        = CFG_DIR .. "/hotel_log.txt"
HT.MAX_ENTRIES_MEM = 2000
HT.MAX_LINES_FILE  = 3000
St.hotelEntries = St.hotelEntries or {} -- {date="YYYY-MM-DD", time="HH:MM", hours, auto=true/false}, новые сверху
St.hotelLogLoaded = false

function HT.loadLog()
    St.hotelEntries = {}
    local allLines = readLogTail(HT.LOG_FILE, HT.MAX_LINES_FILE, 32)
    if #allLines > HT.MAX_LINES_FILE then
        local keepFrom = #allLines - HT.MAX_LINES_FILE + 1
        local trimmed = {}
        for i = keepFrom, #allLines do trimmed[#trimmed+1] = allLines[i] end
        allLines = trimmed
        pcall(function()
            ensureCfgDir()
            local out = io.open(HT.LOG_FILE, "w")
            if out then
                for _, ln in ipairs(allLines) do out:write(ln, "\n") end
                out:close()
            end
    ...
    -- формат строки: "YYYY-MM-DD|HH:MM|hours|auto(0/1)"
    for i = #allLines, 1, -1 do
        local d, t, hrs, auto = allLines[i]:match(
            "^(%d%d%d%d%-%d%d%-%d%d)|(%d%d:%d%d)|(-?%d+)|([01])$")
        if d then
            table.insert(St.hotelEntries, {
                date = d, time = t, hours = tonumber(hrs) or 0, auto = (auto == "1"),
            })
            if #St.hotelEntries >= HT.MAX_ENTRIES_MEM then break end
        end
    end
    St.hotelLogLoaded = true
end

-- добавляет одну запись о продлении отеля в память и дописывает в файл
-- на диске; ничего не делает, если hours <= 0 (нечего логировать)
function HT.addEntry(isAuto, hours)
    hours = math.floor((hours or 0) + 0.5)
    if hours <= 0 then return end
    if not St.hotelLogLoaded then HT.loadLog() end
    local d = os.date("%Y-%m-%d")
    local t = os.date("%H:%M")
    table.insert(St.hotelEntries, 1, {
        date = d, time = t, hours = hours, auto = isAuto and true or false,
    })
    while #St.hotelEntries > HT.MAX_ENTRIES_MEM do table.remove(St.hotelEntries) end
    pcall(function()
        ensureCfgDir()
        local f = io.open(HT.LOG_FILE, "a")
        if f then
            f:write(d .. "|" .. t .. "|" .. hours .. "|" .. (isAuto and "1" or "0") .. "\n")
            f:close()
        end
    end)
    -- ── по просьбе: карточка "Своя оплата" во вкладке "Отель" (см.
    -- drawHotelInner) должна показывать время/кол-во часов ПОСЛЕДНЕГО
    -- продления, независимо от того, открыт ли попап лога — держим это
    -- в cfg (тот же приём, что и taxLastPayTime/taxLastPayAmount) ──
    cfg.hotelLastPayTime  = os.time()
    cfg.hotelLastPayHours = hours
    pcall(saveCfg)
end

-- открывает телефон на домашнем экране; дальше сам onShowDialog ищет
-- иконку "Отель" среди пришедших диалогов (см. ниже) — тот же приём,
-- что и openTaxAppDirect() для налогов, ДО того как у налогов появился
-- прямой запуск по launchedApp|N. Для отеля такой прямой способ пока не
-- делаем (см. комментарий у HOTEL_HOME_DIALOG_ID выше — нет
-- подтверждённого номера именно для пакета "launchedApp").
function openHotelAppDirect()
    pcall(sampCloseCurrentDialog, -1)
    sendPhoneBytes({220, 0, 80, 64}) -- открыть телефон (домашний экран)
end

-- "Можно продлить на: 52 часов" -> 52
function parseHotelRemaining(text)
    local n = (text or ""):match("[Пп]" .. "\xf0\xee\xe4\xeb\xe8\xf2\xfc" .. "%s+\xed\xe0%s*:?%s*(%d+)")
    return n and tonumber(n) or nil
end

-- строки вида "1. 12 часов" -> {[12]=индекс_в_списке(0-based), [24]=..., ...}
function parseHotelOptions(lines)
    local opts = {}
    for i, line in ipairs(lines) do
        local hrs = line:match("^%s*%d+%.%s*(%d+)%s*\xf7\xe0\xf1") -- "N. <часы> час..."
        if hrs then opts[tonumber(hrs)] = i - 1 end
    end
    return opts
end

-- самый большой вариант из списка, который не превышает remaining
function pickHotelOption(opts, remaining)
    local bestHours, bestIndex = nil, nil
    for hrs, idx in pairs(opts) do
        if hrs <= remaining and (not bestHours or hrs > bestHours) then
            bestHours, bestIndex = hrs, idx
        end
    end
    return bestHours, bestIndex
end

-- запускает автопродление отеля: открывает телефон и переводит
-- state-машину в режим ожидания (см. hotel-блок внутри sampev.onShowDialog)
function payHotelNow(isAuto)
    if _hotelState ~= 0 then
        if not isAuto then
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                "\xd3\xe6\xe5\x20\xe2\xfb\xef\xee\xeb\xed\xff\xe5\xf2\xf1\xff\x2c\x20\xef\xee\xe4\xee\xe6\xe4\xe8\xf2\xe5\x2e", -1)
        end
        return
    end
    if _phoneOpBusy then
        if not isAuto then
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                "\xd2\xe5\xeb\xe5\xf4\xee\xed\x20\xe7\xe0\xed\xff\xf2\x20\xe4\xf0\xf3\xe3\xee\xe9\x20\xee\xef\xe5\xf0\xe0\xf6\xe8\xe5\xe9\x2c\x20\xef\xee\xef\xf0\xee\xe1\xf3\xe9\xf2\xe5\x20\xf7\xe5\xf0\xe5\xe7\x20\xf1\xe5\xea\xf3\xed\xe4\xf3", -1)
        end
        return
    end
    if not isPlayerActuallySpawned() then
    ...
    _hotelNavAttempts = 0
    _hotelSessionHours = 0
    local okOpen = pcall(openHotelAppDirect)
    if not okOpen then
        _hotelState = 0
        _phoneOpBusy = false
        pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
            "\xed\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xee\xf2\xea\xf0\xfb\xf2\xfc\x20\xf2\xe5\xeb\xe5\xf4\xee\xed\x20\xce\xf2\xe5\xeb\xfc", -1)
        return
    end
    lua_thread.create(function()
        local waited = 0
        local resent = false
        while _hotelState ~= 0 and waited < HOTEL_TIMEOUT_SEC * 1000 do
            wait(100); waited = waited + 100
            -- ФИКС (та же причина, что и у аналогичного ретрая в
            -- payTaxesNow): если за половину таймаута ни один диалог так
            -- и не перевёл _hotelState дальше 1, первый пакет открытия
            -- телефона/запуска приложения мог потеряться — пробуем ещё раз
            if not resent and _hotelState == 1 and waited >= math.floor(HOTEL_TIMEOUT_SEC * 1000 / 2) then
                resent = true
                pcall(openHotelAppDirect)
            end
        end
        if _hotelState ~= 0 then
            _hotelState = 0
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                "\xec\xe5\xed\xfe\x20\xee\xf2\xe5\xeb\xff\x20\xed\xe5\x20\xee\xf2\xea\xf0\xfb\xeb\xee\xf1\xfc\x20\xe2\xee\xe2\xf0\xe5\xec\xff", -1)
            -- если ДО таймаута успели продлить хотя бы один шаг (сеанс не
            -- нулевой), всё равно пишем это в лог — иначе реально оплаченные
            -- часы молча пропадут из истории
            pcall(HT.addEntry, _hotelIsAuto, _hotelSessionHours)
            _hotelSessionHours = 0
            pcall(sampCloseCurrentDialog, -1)
            wait(150)
            pcall(sampCloseCurrentDialog, -1)
...
-- ФИКС (по просьбе "кнопка оплаты налогов должна оплачивать и отель
-- тоже"): раньше кнопка/команда "оплатить налоги" запускала ТОЛЬКО
-- payTaxesNow(), а отель нужно было оплачивать отдельной кнопкой на
-- своей вкладке. Теперь один вызов делает оба шага ПОСЛЕДОВАТЕЛЬНО
-- (не одновременно — у обоих общий лок телефона _phoneOpBusy, поэтому
-- параллельно они всё равно не смогут отработать, а просто станет в
-- очередь со скучным сообщением об ошибке): сначала налоги, и только
-- когда весь их процесс полностью завершится (и state, и лок телефона
-- сброшены — ждём оба, поэтому таймаут больше, чем у одной оплаты
-- налогов, с запасом на неудачный сценарий), запускается payHotelNow().
-- Если отелю платить нечего/рано — это определит сам payHotelNow(), как
-- и раньше (по кнопке/автоматике).
function payTaxesThenHotel(isAuto)
    payTaxesNow(isAuto)
    lua_thread.create(function()
        local waited = 0
        local maxWait = (TAX_TIMEOUT_SEC + 6) * 1000
        while (_taxState ~= 0 or _phoneOpBusy) and waited < maxWait do
            wait(200); waited = waited + 200
        end
        -- небольшая пауза, чтобы клиент точно доиграл закрытие телефона
        -- после налогов, прежде чем открывать его заново для отеля
        wait(300)
        pcall(payHotelNow, isAuto)
    end)
end
...
lua_thread.create(function()
    while true do
        ...
        -- ── автопродление отеля по кулдауну (та же схема, что и у
        -- автооплаты налогов выше) ──
        pcall(function()
            if cfg.hotelAutoEnabled and _hotelState == 0 and isPlayerActuallySpawned() then
                local intervalSec = math.max(1, tonumber(cfg.hotelAutoIntervalHours) or 24) * 3600
                if cfg.hotelLastPayTime == 0 or (os.time() - cfg.hotelLastPayTime) >= intervalSec then
                    pcall(sampAddChatMessage, "{FFD700}[PC Stats] " ..
                        "\xc0\xe2\xf2\xee\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5\x3a\x20\xe7\xe0\xef\xf3\xf1\xea\xe0\xfe\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5\x20\xed\xee\xec\xe5\xf0\xe0\x2e\x2e\x2e", -1)
                    payHotelNow(true)
                end
            end
        end)
...
local function drawColorRowInline(titleU8, rBuf, gBuf, bBuf, cfgRKey, cfgGKey, cfgBKey, defR, defG, defB, whiteChrome, uidSeed)
...
local function drawSettingsInner(h, sw, sh)
    ...
    -- ФИКС (по просьбе): блок "Готовые темы" (6 кнопок-пресетов) убран
    -- совсем — акцент/фон строк/обводка настраиваются вручную ниже
    -- ("Свободно" / "Акцент", "Фон строк" и т.п.), а быстрый общий цвет
    -- задаёт кнопка "Случайный цвет" дальше по вкладке.
    imgui.Dummy(imgui.ImVec2(0, S(6)))

    -- ФИКС "вид не меняется": та же причина, что и в
        -- drawSimpleColorPicker выше — whiteChrome=true держал полоску
        -- всегда белой независимо от выбранного цвета. Теперь тоже
        -- показывает реальный текущий цвет ──
        drawColorRowInline(u8"\xc0\xea\xf6\xe5\xed\xf2\x20\x28\xf1\xe2\xee\xe9\x20\xf6\xe2\xe5\xf2\x29", St.custRbuf, St.custGbuf, St.custBbuf,
            "custR","custG","custB", getTheme().acc[1],getTheme().acc[2],getTheme().acc[3], false, "accentColor")

        drawColorRowInline(u8"\xd4\xee\xed\x20\xf1\xf2\xf0\xee\xea", St.rowBgRbuf, St.rowBgGbuf, St.rowBgBbuf,
            "rowBgR","rowBgG","rowBgB", getTheme().acc[1]*0.35,getTheme().acc[2]*0.35,getTheme().acc[3]*0.35, false, "rowBgColor")

    -- ── цвет текста / фон скрипта / обводка: те же квадратик+кнопка+пикер,
    -- что и у "Свой цвет"/"Фон строк" выше, но управляют imgui.Col.Text,
    -- imgui.Col.WindowBg и imgui.Col.Border глобально (по просьбе) ──
    drawSimpleColorPicker(u8"\xd6\xe2\xe5\xf2\x20\xf2\xe5\xea\xf1\xf2\xe0", "textColor",
        St.textRbuf, St.textGbuf, St.textBbuf, "textR","textG","textB", "textPickerVec", 1.0,1.0,1.0)

    -- ФИКС (по просьбе): раньше тут был тумблер "Скрипт белый/чёрный"
    -- (cfg.uiLightMode), который убрали совсем — принудительно
    -- возвращаем обычную (тёмную) тему на случай, если у кого-то был
    -- включён белый фон.
    if cfg.uiLightMode then cfg.uiLightMode = false; saveCfg() end

    -- ── по просьбе: простой пикер фона меню — той же системой, что и
    -- "Цвет фона"/"Цвет текста" у всплывающих уведомлений (один
    -- ColorEdit3, применяется сразу) — вместо старого тумблера
    -- "чёрный/белый скрипт". cfg.winBgR/G/B уже существовали в
    -- конфиге, но нигде не были подключены к отрисовке — теперь
    -- applyStyle() их реально использует (см. imgui.Col.WindowBg там же)
    imgui.Spacing()
    do
        if not St.menuBgBuf then
            local hasCustom = cfg.winBgR and cfg.winBgR >= 0
            St.menuBgBuf = imgui.new.float[3](
                hasCustom and cfg.winBgR or 0.0,
                hasCustom and cfg.winBgG or 0.0,
                hasCustom and cfg.winBgB or 0.0)
        end
        imgui.TextColored(thDim(), u8"\xd4\xee\xed\x20\xec\xe5\xed\xfe")
        imgui.SetNextItemWidth(imgui.GetContentRegionAvail().x)
        if imgui.ColorEdit3("##menuBgColor", St.menuBgBuf) then
            cfg.winBgR, cfg.winBgG, cfg.winBgB = St.menuBgBuf[0], St.menuBgBuf[1], St.menuBgBuf[2]
            saveCfg()
        end
    end

    drawSimpleColorPicker(u8"\xce\xe1\xe2\xee\xe4\xea\xe0", "outlineColor",
        St.outlineRbuf, St.outlineGbuf, St.outlineBbuf, "outlineR","outlineG","outlineB", "outlinePickerVec",
        St.outlineRbuf[0], St.outlineGbuf[0], St.outlineBbuf[0])

    -- ── цвет текста сообщений скрипта в чате: подменяет ведущий SAMP-тег
    -- {RRGGBB} у сообщений скрипта (см. applyCustomChatColor выше), кроме
    -- красных тегов ошибок/предупреждений ──
    drawSimpleColorPicker(u8"\xd6\xe2\xe5\xf2\x20\xf2\xe5\xea\xf1\xf2\xe0\x20\xe2\x20\xf7\xe0\xf2\xe5", "chatColor",
        St.chatRbuf, St.chatGbuf, St.chatBbuf, "chatR","chatG","chatB", "chatPickerVec", 0.0, 1.0, 0.53)

    -- ── случайный цвет: одна кнопка внизу вкладки — генерирует случайный
    -- акцент, фон строк, цвет текста, фон скрипта и обводку и сразу
    -- применяет всё вместе (по просьбе) ──
    imgui.Dummy(imgui.ImVec2(0, S(6)))
...
local function drawTaxPopupsGlobal()
    ...
    -- ── попап календаря ──
    do
        -- ФИКС (по просьбе "чтоб в логах было видно оплаты отеля"): даты
        -- отеля теперь тоже подсвечиваются в календаре, вместе с датами
        -- оплат налогов — единый календарь на обе истории ──
        if not St.hotelLogLoaded then HT.loadLog() end
        local taxDates = {}
        for _, e in ipairs(St.taxEntries or {}) do taxDates[e.date] = true end
        for _, e in ipairs(St.hotelEntries or {}) do taxDates[e.date] = true end
    ...
    if St._taxLogEntriesPopupOpen then
        ...
        local okLog, errLog = pcall(function()
            if imgui.BeginPopup("##taxLogEntriesPopup") then
                ...
                -- ── по просьбе: отдельным блоком карточек, той же датой,
                -- показываем ещё и продления отеля за этот день (отличаются
                -- цветом рамки и текстом "N ч" вместо суммы денег) ──
                local shownHt = 0
                for _, e in ipairs(St.hotelEntries or {}) do
                    if e.date == St._taxLogSelectedDate then
                        shownHt = shownHt + 1
                        local dl_ht = imgui.GetWindowDrawList()
                        local p_ht = imgui.GetCursorScreenPos()
                        local aw_ht = imgui.GetContentRegionAvail().x
                        local cardH_ht = S(72)
                        dl_ht:AddRectFilled(p_ht, imgui.ImVec2(p_ht.x+aw_ht, p_ht.y+cardH_ht),
                            imgui.ColorConvertFloat4ToU32(iv4(r*0.10,g*0.10,b*0.10,0.92)), 8)
                        dl_ht:AddRect(p_ht, imgui.ImVec2(p_ht.x+aw_ht, p_ht.y+cardH_ht),
                            imgui.ColorConvertFloat4ToU32(iv4(0.25,0.55,0.95,0.55)), 8, 0, 1.2)
                        imgui.SetCursorScreenPos(imgui.ImVec2(p_ht.x + S(14), p_ht.y + S(8)))
                        imgui.TextColored(iv4(1,1,1,1), e.time .. "  \xe2\x80\x94  " ..
                            u8"\xce\xf2\xe5\xeb\xfc\x3a\x20" .. tostring(e.hours) .. "\x20\xf7")
                        imgui.SetCursorScreenPos(imgui.ImVec2(p_ht.x + S(14), p_ht.y + S(32)))
                        imgui.TextColored(thDim(), u8"\xc2\xf0\xe5\xec\xff\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xff\x3a\x20" ..
                            (e.auto and u8"\xe0\xe2\xf2\xee\xec\xe0\xf2\xe8\xf7\xe5\xf1\xea\xe8" or u8"\xe2\xf0\xf3\xf7\xed\xf3\xfe"))
                        imgui.SetCursorScreenPos(imgui.ImVec2(p_ht.x + S(14), p_ht.y + S(50)))
                        imgui.TextColored(thGreen(), u8"\xd1\xf2\xe0\xf2\xf3\xf1\x3a\x20\xd3\xf1\xef\xe5\xf8\xed\xee")
                        imgui.SetCursorScreenPos(imgui.ImVec2(p_ht.x, p_ht.y + cardH_ht + S(8)))
                    end
                end
                if shownTx == 0 and shownHt == 0 then
...
local function drawTaxes(h)
    ...
    -- ФИКС (по просьбе "перенести все функции вкладки 'Отель' сюда"):
    -- содержимое вкладки "Отель" (drawHotelInner) теперь рисуется сразу
    -- следом, ВНУТРИ того же дочернего окна "##staxes" — оно тут же
    -- прокручивается одним общим списком вместе с налогами (отдельного
    -- BeginChild/EndChild для отеля здесь больше нет, поэтому парность
    -- Begin/End не нарушается). Каждый блок — свой pcall, чтобы ошибка
    -- в одном не срывала отрисовку другого и не портила imgui-стек.
    if ok then
        imgui.Dummy(imgui.ImVec2(0, S(10)))
        imgui.Separator()
        imgui.Dummy(imgui.ImVec2(0, S(6)))
        local rH, gH, bH = 1.0, 0.65, 0.15
        imgui.TextColored(iv4(rH,gH,bH,1.0), "  "..u8"\xce\xf2\xe5\xeb\xfc")
        imgui.Dummy(imgui.ImVec2(0, S(6)))
        local okH, errH = pcall(drawHotelInner, h)
        if not okH then
            St._taxLogEntriesPopupOpen = false
            St._taxLogEntriesOpenedOnce = false
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                "\xee\xf8\xe8\xe1\xea\xe0\x20\xee\xf2\xf0\xe8\xf1\xee\xe2\xea\xe8\x20\xf0\xe0\xe7\xe4\xe5\xeb\xe0\x20\xce\xf2\xe5\xeb\xfc: " .. tostring(errH), -1)
        end
    end
    imgui.EndChild()
    imgui.PopStyleColor()
...
-- ============================================================
--  ВКЛАДКА "ОТЕЛЬ" (по просьбе: своя кнопка ручного продления + авто-
--  продление по кулдауну + лог — та же структура, что и у "Налоги"
--  выше, только вместо денег/суммы — часы продления, и своя кнопка
--  вместо общей с налогами). Попап лога/календаря переиспользуется
--  ТОТ ЖЕ, что и у "Налоги" (drawTaxPopupsGlobal), т.к. он уже
--  показывает записи и налогов, и отеля вместе по датам ──
-- ============================================================
function drawHotelInner(h)
    local r, g, b = getAcc()

    -- ── карточка "Своя оплата" — последнее продление отеля ──
    do
        local dl_sp = imgui.GetWindowDrawList()
        local p_sp  = imgui.GetCursorScreenPos()
        local aw_sp = imgui.GetContentRegionAvail().x
        local hasHrs = cfg.hotelLastPayHours and cfg.hotelLastPayHours > 0
        local cardH_sp = hasHrs and S(64) or S(44)
        dl_sp:AddRectFilled(p_sp, imgui.ImVec2(p_sp.x+aw_sp, p_sp.y+cardH_sp),
            imgui.ColorConvertFloat4ToU32(iv4(r*0.08,g*0.08,b*0.08,0.85)), 10)
        dl_sp:AddRect(p_sp, imgui.ImVec2(p_sp.x+aw_sp, p_sp.y+cardH_sp),
            imgui.ColorConvertFloat4ToU32(iv4(r,g,b,0.55)), 10, 0, 1.3)
        imgui.SetCursorScreenPos(imgui.ImVec2(p_sp.x + S(12), p_sp.y + S(9)))
        imgui.TextColored(thAcc(), u8"\xd1\xe2\xee\xb8\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5")
        imgui.SetCursorScreenPos(imgui.ImVec2(p_sp.x + S(12), p_sp.y + S(28)))
        imgui.TextColored(iv4(1,1,1,1), fmtTaxTime(cfg.hotelLastPayTime)..fmtTaxAgo(cfg.hotelLastPayTime))
        if hasHrs then
            imgui.SetCursorScreenPos(imgui.ImVec2(p_sp.x + S(12), p_sp.y + S(46)))
            imgui.TextColored(thGold(), tostring(cfg.hotelLastPayHours) .. " " .. u8"\xf7")
        end
        imgui.SetCursorScreenPos(imgui.ImVec2(p_sp.x, p_sp.y + cardH_sp + S(8)))
    end

    -- ── лог/календарь — переиспользуем тот же попап, что и у "Налоги" ──
    imgui.Separator()
    imgui.Dummy(imgui.ImVec2(0, S(6)))
    imgui.TextColored(thAcc(), u8"  \xc8\xf1\xf2\xee\xf0\xe8\xff\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe9")
    imgui.Spacing()
    if not St.taxLogLoaded then TX.loadLog() end
    if not St.hotelLogLoaded then HT.loadLog() end
    ...
    do
        ...
        imgui.PushStyleColor(imgui.Col.ButtonHovered, iv4(r*0.42,g*0.42,b*0.42,1.0))
        imgui.PushStyleColor(imgui.Col.ButtonActive,  iv4(r*0.60,g*0.60,b*0.60,1.0))
        do local _pbHL = prettyBtnPush(8.0)
        St._taxLogBtnScreenPos = imgui.GetCursorScreenPos()
        if imgui.Button(u8"\xcb\xee\xe3\xe8\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe9##hotelOpenLogsModal", imgui.ImVec2(-1, S(30))) then
            if St._taxLogSelectedDate then
                St._taxLogEntriesPopupOpen = true
            else
                St._taxCalendarPopupOpen = true
        ...
        prettyBtnPop(_pbHL) end
        imgui.PopStyleColor(3)
    end
    imgui.Spacing()

    local _hIntervalSec = math.max(1, tonumber(cfg.hotelAutoIntervalHours) or 24) * 3600
    local _hSinceLastPay = (cfg.hotelLastPayTime ~= 0) and (os.time() - cfg.hotelLastPayTime) or nil
    local _hDueInSec = _hSinceLastPay and math.max(0, _hIntervalSec - _hSinceLastPay) or nil

    imgui.Dummy(imgui.ImVec2(0, S(6)))

    -- ── отдельная кнопка ручного продления отеля ──
    imgui.PushStyleColor(imgui.Col.Button,        iv4(r*0.55,g*0.55,b*0.55,1.0))
    imgui.PushStyleColor(imgui.Col.ButtonHovered, iv4(r*0.78,g*0.78,b*0.78,1.0))
    imgui.PushStyleColor(imgui.Col.ButtonActive,  iv4(r,g,b,1.0))
    if imgui.Button(u8"  \xce\xef\xeb\xe0\xf2\xe8\xf2\xfc\x20\xee\xf2\xe5\xeb\xfc\x20\xf1\xe5\xe9\xf7\xe0\xf1  ", imgui.ImVec2(-1, S(38))) then
        payHotelNow(false)
    end
    if imgui.IsItemHovered and imgui.IsItemHovered() then
        pcall(function()
            imgui.BeginTooltip()
            imgui.TextColored(iv4(0.75,0.80,0.90,1.0), u8"\xce\xf2\xea\xf0\xee\xe5\xf2\x20\xf2\xe5\xeb\xe5\xf4\xee\xed\x20\xe8\x20\xef\xf0\xff\xec\xee\x20\xf1\xe5\xe9\xf7\xe0\xf1\x20\xef\xf0\xee\xe4\xeb\xe8\xf2\x20\xed\xee\xec\xe5\xf0\x20\xe2\x20\xee\xf2\xe5\xeb\xe5")
            imgui.EndTooltip()
        end)
    end
    imgui.PopStyleColor(3)
    imgui.TextColored(thDim(), "  "..u8"\xce\xf2\xea\xf0\xee\xe5\xf2\x20\xf2\xe5\xeb\xe5\xf4\xee\xed\x20\xe8\x20\xef\xf0\xee\xe4\xeb\xe8\xf2\x20\xed\xee\xec\xe5\xf0\x20\xed\xe0\x20\xec\xe0\xea\xf1\xe8\xec\xf3\xec")

    if _hotelState ~= 0 then
        imgui.Dummy(imgui.ImVec2(0, S(6)))
        imgui.TextColored(thGold(), "  \xe2\x8f\xb3 "..u8"\xc8\xe4\xb8\xf2\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5\x20\xed\xee\xec\xe5\xf0\xe0\x2e\x2e\x2e")
        imgui.TextColored(thDim(),  "  "..u8"\xce\xf2\xea\xf0\xfb\xe2\xe0\xfe\x20\xf2\xe5\xeb\xe5\xf4\xee\xed\x20\xe8\x20\xe8\xf9\xf3\x20\xed\xf3\xe6\xed\xfb\xe9\x20\xef\xf3\xed\xea\xf2\x20\xec\xe5\xed\xfe")
    end
    ...
    -- ── автопродление (тумблер + кулдаун, минимум до 5 суток — та же
    -- логика и диапазон, что и у автооплаты налогов) ──
    imgui.TextColored(thAcc(), u8"  \xc0\xe2\xf2\xee\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5")
    imgui.Spacing()
    taxToggleRow("##hotelAutoSw", u8"\xc2\xea\xeb\xfe\xf7\xe8\xf2\xfc\x20\xe0\xe2\xf2\xee\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5", cfg.hotelAutoEnabled,
        function(v) cfg.hotelAutoEnabled = v; St.hotelAutoBuf[0] = v end,
        u8"")

    if cfg.hotelAutoEnabled then
        if not _hSinceLastPay then
            imgui.TextColored(thGold(), "  "..u8"")
        elseif _hDueInSec and _hDueInSec > 0 then
            imgui.TextColored(thGold(), "  "..u8"\xd1\xeb\xe5\xe4\xf3\xfe\xf9\xe0\xff\x20\xef\xf0\xee\xe2\xe5\xf0\xea\xe0\x20\xf7\xe5\xf0\xe5\xe7\x3a\x20"..fmtDuration(_hDueInSec))
        else
            imgui.TextColored(thGold(), "  "..u8"\xd1\xf0\xee\xea\x20\xef\xee\xe4\xee\xf8\xb8\xeb\x20\x97\x20\xef\xf0\xee\xe2\xe5\xf0\xea\xe0\x20\xf0\xe0\xe7\x20\xe2\x20\xec\xe8\xed\xf3\xf2\xf3")
        end
        imgui.Dummy(imgui.ImVec2(0, S(4)))
    ...
    imgui.TextColored(thDim(), "  "..u8"\xc8\xed\xf2\xe5\xf0\xe2\xe0\xeb")
    imgui.SameLine(0, S(6))
    local _hIvHrs = St.hotelIntervalBuf[0]
    local _hIvLbl = tostring(_hIvHrs) .. " " .. u8"\xf7"
    if _hIvHrs >= 24 then
        local _hIvDays = _hIvHrs / 24
        local _hIvDaysStr = (math.floor(_hIvDays) == _hIvDays) and tostring(math.floor(_hIvDays))
            or string.format("%.1f", _hIvDays)
        _hIvLbl = _hIvLbl .. "  (" .. _hIvDaysStr .. " " .. u8"\xe4\xed" .. ")"
    end
    imgui.TextColored(thAcc(), _hIvLbl)
    imgui.PushStyleColor(imgui.Col.FrameBg,        iv4(r*0.16,g*0.16,b*0.16,1.0))
    imgui.PushStyleColor(imgui.Col.FrameBgHovered, iv4(r*0.24,g*0.24,b*0.24,1.0))
    imgui.PushStyleColor(imgui.Col.FrameBgActive,  iv4(r*0.30,g*0.30,b*0.30,1.0))
    imgui.PushStyleColor(imgui.Col.SliderGrab,      iv4(r,g,b,1.0))
    imgui.PushStyleColor(imgui.Col.SliderGrabActive,iv4(math.min(1,r*1.2),math.min(1,g*1.2),math.min(1,b*1.2),1.0))
    imgui.PushItemWidth(-1)
    if imgui.SliderInt('##hotelInterval', St.hotelIntervalBuf, 1, 120, "%d "..u8"\xf7") then
        cfg.hotelAutoIntervalHours = St.hotelIntervalBuf[0]
        saveCfg()
    end
    imgui.PopItemWidth()
    imgui.PopStyleColor(5)
...
function drawHotel(h)
    imgui.PushStyleColor(imgui.Col.ChildBg, iv4(0,0,0,0))
    imgui.BeginChild("##shotel", imgui.ImVec2(0, h), false)
    local ok, err = pcall(drawHotelInner, h)
    imgui.EndChild()
    imgui.PopStyleColor()
    if not ok then
        St._taxLogEntriesPopupOpen = false
        St._taxLogEntriesOpenedOnce = false
        pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
            "\xee\xf8\xe8\xe1\xea\xe0\x20\xee\xf2\xf0\xe8\xf1\xee\xe2\xea\xe8\x20\xe2\xea\xeb\xe0\xe4\xea\xe8\x20\xce\xf2\xe5\xeb\xfc: " .. tostring(err), -1)
    end
end
        ...
        do
            if St.activeTab == 5 then
                ...
                if St.activeTab == 6 then
                    -- вкладка "Налоги" (теперь объединена с "Отель" — см.
                    -- SECTION_DEFS/drawTaxes выше): кнопка "Сброс" одним
                    -- нажатием возвращает к дефолту тумблеры И налогов, И
                    -- отеля (раньше это были две разные кнопки на двух
                    -- разных вкладках — вкладки "Отель" отдельно больше нет)
                    ...
                    if imgui.Button(u8"  \xd1\xe1\xf0\xee\xf1  ##taxReset", imgui.ImVec2(bw, S(40))) then
                        ...
                        cfg.hotelAutoEnabled       = false
                        cfg.hotelAutoIntervalHours = 24
                        St.hotelAutoBuf[0]         = false
                        St.hotelIntervalBuf[0]     = 24
                        saveCfg()
                    end
                    if imgui.IsItemHovered and imgui.IsItemHovered() then
                        pcall(function()
                            imgui.BeginTooltip()
                            imgui.TextColored(iv4(0.75,0.80,0.90,1.0), u8"")
                            imgui.EndTooltip()
                        end)
                    end
                    imgui.PopStyleColor(3)
                elseif St.activeTab == 7 then
                    -- ФИКС: вкладка "Отель" больше не выбирается через UI (кнопки
                    -- нет в SECTION_DEFS), но ветка оставлена нетронутой на случай
                    -- устаревшего cfg.lastTab==7 из старой версии — сброс делает
                    -- то же самое, что и объединённая кнопка на вкладке "Налоги" выше
                    imgui.PushStyleColor(imgui.Col.Button,        iv4(0.55,0.12,0.12,1.0))
                    imgui.PushStyleColor(imgui.Col.ButtonHovered, iv4(0.78,0.18,0.18,1.0))
                    imgui.PushStyleColor(imgui.Col.ButtonActive,  iv4(1.0, 0.25,0.25,1.0))
                    if imgui.Button(u8"  \xd1\xe1\xf0\xee\xf1  ##hotelReset", imgui.ImVec2(bw, S(40))) then
                        cfg.hotelAutoEnabled       = false
                        cfg.hotelAutoIntervalHours = 24
                        St.hotelAutoBuf[0]         = false
                        St.hotelIntervalBuf[0]     = 24
                    ...
                    if imgui.IsItemHovered and imgui.IsItemHovered() then
                        pcall(function()
                            imgui.BeginTooltip()
                            imgui.TextColored(iv4(0.75,0.80,0.90,1.0), u8"")
                            imgui.EndTooltip()
                        end)
                    end
                    imgui.PopStyleColor(3)
...
function sampev.onShowDialog(id, style, title, btn1, btn2, text)
    ...
    -- ── автопродление отеля (см. payHotelNow) ──
    if _hotelState ~= 0 then
        local handledHotel = false
        pcall(function()
            -- ФИКС: id экрана продления фиксирован (26148) — как только он
            -- пришёл, обрабатываем его сразу, независимо от того, в каком
            -- именно под-шаге навигации сейчас _hotelState (1 или 2) — та же
            -- логика, что и с id-ограничением у налогов: полагаться на id
            -- каждого промежуточного экрана телефона ненадёжно
            if id == HOTEL_DIALOG_ID then
                _hotelState = 2
                local lines = splitTaxLines(text)
                local remaining = parseHotelRemaining(text)
                local opts = parseHotelOptions(lines)
                _hotelLastOpts = opts
                if not remaining then
                    -- не смогли распознать остаток часов — не гадаем, просто
                    -- останавливаемся, чтобы не нажать что-то не то
                    _hotelState = 0
                    pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                        "\xed\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xf0\xe0\xf1\xef\xee\xe7\xed\xe0\xf2\xfc\x20\xfd\xea\xf0\xe0\xed\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xff\x20\xee\xf2\xe5\xeb\xff", -1)
                    lua_thread.create(function()
                        pcall(sampCloseCurrentDialog, -1)
                        wait(150)
                        pcall(sampCloseCurrentDialog, -1)
                ...
                local hrs, idx = pickHotelOption(opts, remaining)
                if not hrs then
                    -- остатка не хватает даже на минимальный вариант из списка —
                    -- платить больше нечем, закрываем меню и пишем итог
                    _hotelState = 0
                    pcall(sampAddChatMessage, "{00FF88}[PC Stats] " ..
                        "\xce\xef\xeb\xe0\xf2\xe0\x20\xee\xf2\xe5\xeb\xff\x20\xf3\xf1\xef\xe5\xf8\xed\xee\x20\xe7\xe0\xe2\xe5\xf0\xf8\xe5\xed\xe0\x2c\x20\xee\xf1\xf2\xe0\xeb\xee\xf1\xfc\x20" ..
                        tostring(remaining) .. "\x20\xf7\x2e", -1)
                    pcall(pcs_notify, u8"\xce\xef\xeb\xe0\xf2\xe0\x20\xee\xf2\xe5\xeb\xff\x20\xf3\xf1\xef\xe5\xf8\xed\xee\x20\xe7\xe0\xe2\xe5\xf0\xf8\xe5\xed\xe0", "success")
                    -- ── по просьбе: пишем оплату отеля в лог (как и у налогов),
                    -- чтобы в истории было видно, сколько часов и когда продлили ──
                    pcall(HT.addEntry, _hotelIsAuto, _hotelSessionHours)
                    _hotelSessionHours = 0
                    lua_thread.create(function()
                        pcall(sampCloseCurrentDialog, -1)
                ...
                local curDialog = id
                _hotelSessionHours = (_hotelSessionHours or 0) + hrs
                pcall(sampAddChatMessage, "{FFD700}[PC Stats] " ..
                    "\xce\xef\xeb\xe0\xf7\xe8\xe2\xe0\xfe\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5\x20\xee\xf2\xe5\xeb\xff\x20\xed\xe0\x20" ..
                    tostring(hrs) .. "\x20\xf7\x2e\x20\x28\xee\xf1\xf2\xe0\xeb\xee\xf1\xfc\x20" .. tostring(remaining) .. "\x20\xf7\x2e\x29", -1)
                lua_thread.create(function()
                    wait(HOTEL_STEP_DELAY)
                    pcall(sampSendDialogResponse, curDialog, 1, idx, "")
                    wait(HOTEL_POST_PAY_WAIT_MS)
                    -- ФИКС (по просьбе "должен снова открыть меню и оплатить
                    -- сколько осталось"): пока остаётся хотя бы минимальный
                    -- вариант из списка, открываем то же меню отеля заново —
                    -- сервер пришлёт свежий id=26148 с уже обновлённым
                    -- остатком часов, и вся обработка выше повторится сама
                    if _hotelState ~= 0 then
                        pcall(openHotelAppDirect)
                    end
                end)
            ...
            if _hotelState == 1 then
                -- ждём иконку "Отель" на текущем экране телефона (домашний
                -- экран или промежуточная папка) — та же схема, что и у
                -- иконки "Налоги"/"Банк" выше
                local lines = splitTaxLines(text)
                if _hotelNavAttempts < HOTEL_NAV_MAX_ATTEMPTS then
                    for i, line in ipairs(lines) do
                        if line:find(HOTEL_ICON_NEEDLE, 1, true) then
                            _hotelNavAttempts = _hotelNavAttempts + 1
                            local curDialog, curIndex = id, i - 1
                            lua_thread.create(function()
                                wait(HOTEL_STEP_DELAY)
                                pcall(sampSendDialogResponse, curDialog, 1, curIndex, "")
        ...
        if handledHotel then return false end
    end
...
function sampev.onServerMessage(color, text)
    -- ── страховка на случай ошибки сервера при продлении отеля (см.
    -- payHotelNow): по идее наш выбор варианта всегда укладывается в
    -- лимит "Можно продлить на", но если сервер всё же прислал отказ
    -- ("[Ошибка] ... максимум - N часов", например из-за гонки с другим
    -- источником продления отеля игроком/другим скриптом в это же время),
    -- не оставляем автоматику висеть — сразу останавливаемся и снимаем
    -- блокировку телефона, чтобы не зациклиться на постоянных отказах ──
    pcall(function()
        if _hotelState == 0 then return end
        local clean = stripColor(text or "")
        if clean:find(HOTEL_ERR_WORD_1, 1, true) and clean:find(HOTEL_ERR_WORD_2, 1, true) then
            _hotelState = 0
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " .. clean, -1)
            pcall(sampAddChatMessage, "{FF6666}[PC Stats] " ..
                "\xf1\xe5\xf0\xe2\xe5\xf0\x20\xee\xf2\xea\xeb\xee\xed\xe8\xeb\x20\xef\xf0\xee\xe4\xeb\xe5\xed\xe8\xe5\x20\xee\xf2\xe5\xeb\xff\x2c\x20\xee\xf1\xf2\xe0\xed\xe0\xe2\xeb\xe8\xe2\xe0\xfe\x20\xe0\xe2\xf2\xee\xee\xef\xeb\xe0\xf2\xf3", -1)
            -- если хотя бы один шаг продления успел пройти до отказа сервера —
            -- всё равно записываем реально оплаченные часы в лог
            pcall(HT.addEntry, _hotelIsAuto, _hotelSessionHours)
            _hotelSessionHours = 0
            lua_thread.create(function()
                pcall(sampCloseCurrentDialog, -1)
...
function main()
    ...
    St.hotelAutoBuf[0]       = cfg.hotelAutoEnabled
    St.hotelIntervalBuf[0]   = cfg.hotelAutoIntervalHours
    ...
    pcall(sampRegisterChatCommand, "payhotel", function() payHotelNow(false) end)
